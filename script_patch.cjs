...
const targetPath = 'public/js/core/events.js';
...
const newString = `              }
              ...
              if (impersonateBtn) {
                  ...
                  modalBox.innerHTML = \`
                      <h3 style="color:var(--text-primary); margin-bottom: 10px;">Ingresar como \${leaderName}</h3>
...
if (normalizedContent.includes(normalizedOldString)) {
    let replacedContent = normalizedContent.replace(normalizedOldString, normalizedNewString);
    fs.writeFileSync(targetPath, replacedContent, 'utf8');
    patched = true;
    console.log('Successfully patched events.js');
}

if (!patched) {
   console.log('Patch failed! Could not find target code snippet.');
}
