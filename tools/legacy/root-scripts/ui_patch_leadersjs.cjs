...
// Patch main ranking row
const oldRankingRowRegex = /const row = \`\s*<tr class="hover:bg-gray-50 transition-colors">[\s\S]*?<\/tr>\s*\`;/g;

const newRankingRow = `const row = \`
...
jsCode = jsCode.replace(oldRankingRowRegex, newRankingRow);

// Patch local ranking row
const oldLocalRowRegex = /const row = \`\s*<tr class="hover:bg-gray-50 transition-colors">[\s\S]*?<\/td>\s*<\/tr>\s*\`;/g;

const newLocalRow = `const row = \`
...
jsCode = jsCode.replace(oldLocalRowRegex, newLocalRow);


fs.writeFileSync('public/assets/js/leaderPerformance.js', jsCode, 'utf8');
console.log('leaderPerformance.js updated for better UI!');
